...
namespace Final_System
{
    public partial class Clientes : Form
    {
        ...
        private void LoadData()
        {
            ...
            string CommandText = "select * from MotorradTB_Clientes";
            DA = new SQLiteDataAdapter(CommandText, sql_con);
            DS.Reset();
            DA.Fill(DS);
            DT = DS.Tables[0];
            DataGrid_Clientes.DataSource = DT;
            sql_con.Close();
        }
        ...
        private void Clientes_MouseUp(object sender, MouseEventArgs e)
        {
            Arrastando = false;
        }
        ...
        private void Clientes_MouseMove(object sender, MouseEventArgs e)
        {
            if (Arrastando == true)
            {
        ...
        {
            string txtQuery = "insert into MotorradTB_Clientes(ID, Nome, Email, Celular, Fixo, Endereco, Numero, DataNascimento, DataCadastro)Values('" + Txt_ID.Text + "','" + Txt_Nome.Text + "','" + Txt_Email.Text + "','" + Txt_Celular.Text + "','" + Txt_Fixo.Text + "','" + Txt_Endereco.Text + "', '" + Txt_Numero.Text + "','" + Txt_DataNascimento.Text + "', '" + Txt_DataCadastro.Text + "')";
            ...
            Txt_DataCadastro.Text = String.Empty;
            
        }

        private void Btn_Editar_Click(object sender, EventArgs e)
        {
            string txtQuery = "update MotorradTB_Clientes set(Nome, Email, Celular, Fixo, Endereco, Numero, DataNascimento, DataCadastro) = ('" + Txt_Nome.Text + "','" + Txt_Email.Text + "','" + Txt_Celular.Text + "','" + Txt_Fixo.Text + "','" + Txt_Endereco.Text + "', '" + Txt_Numero.Text + "','" + Txt_DataNascimento.Text + "', '" + Txt_DataCadastro.Text + "')where ID= '" + Txt_ID.Text + "'";
            ExecuteQuery(txtQuery);
            LoadData();
        ...
        private void Btn_Deletar_Click(object sender, EventArgs e)
        {
            ...
            {
                String txtQuery = "delete from MotorradTB_Clientes where ID= '" + Txt_ID.Text + "'";
                ExecuteQuery(txtQuery);
                LoadData();
        ...
        private void DataGrid_Clientes_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0)
            {
                DataGridViewRow row = this.DataGrid_Clientes.Rows[e.RowIndex];
                ...
                Txt_DataCadastro.Text = row.Cells["DataCadastro"].Value.ToString();
        ...
        public void DisplayValue()
        {
            ...
            DA = new SQLiteDataAdapter("SELECT * from MotorradTB_Clientes", sql_con);
            DT = new DataTable();
            DA.Fill(DT);
            DataGrid_Clientes.DataSource = DT;
            sql_con.Close();
        ...
        {
            SetConnection();
            string query = "SELECT * FROM MotorradTB_Clientes WHERE Nome like '%" + search + "%' or ID like '%" + search + "%' or Email like '%" + search + "%' or Celular like '%" + search + "%' or Fixo like '%" + search + "%' or Endereco like '%" + search + "%' or Numero like '%" + search + "%' or DataNascimento like '%" + search + "%' or DataCadastro like '%" + search + "%'";
            DA = new SQLiteDataAdapter(query, sql_con);
            DT = new DataTable();
            DA.Fill(DT);
            DataGrid_Clientes.DataSource = DT;
            sql_con.Close();
